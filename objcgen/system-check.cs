	...
	public static class SystemCheck
	{
		...
		static void VerifyVersion (string name, string product, Version version)
		{
			...
			if (version < min_version || version > max_version)
				throw ErrorHelper.CreateError (15, $"Could not find a valid version of {product} (found {version}, but at least {min_version} and not more than {max_version} is required).");
